...
let _month = monthName[today.getMonth()]
let weekdays = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
let wday = weekdays[today.getDay() - 1]
document.getElementById('date').innerHTML = wday + " - " + _month + "  " + _day
...
let myStorage = JSON.parse(localStorage.getItem('myTask'))
let _deletedTasks = []
...
function addtsk(tsk) {
    ...
    editBtn.addEventListener('click', () => {
        ...
        innerBtn.onclick = () => {
            ...
            // Update localStorage
            let index = _database.indexOf(tsk)
            if (index !== -1) {
                _database[index] = input.value
                localStorage.setItem('myTask', JSON.stringify(_database))
            }
